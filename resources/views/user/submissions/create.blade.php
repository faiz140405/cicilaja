            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-8 border border-gray-200 dark:border-gray-700 transition-colors duration-300">
                ...
                <form id="submission-form" action="{{ route('user.submissions.store') }}" method="POST" x-data="{ 
                    ...
                    calculateInstallment() {
                        ...
                        let bunga = 0.10; // Bunga Flat 10%
                        
                        if (sisaHarga < 0) {
                            alert('Uang Muka melebihi Harga Kredit!');
                            this.dp = 0; // Reset jika invalid
                ...
                }" @change="calculateInstallment()" x-init="calculateInstallment()"> 
                {{-- Tambahkan x-init agar hitungan awal langsung muncul --}}
                    ...
                    <div class="mb-6">
                        ...
                        <input type="number" name="down_payment" id="down_payment" x-model.number="dp" min="0" max="{{ $product->credit_price }}" required
                               class="mt-1 block w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 rounded-md shadow-sm p-2.5 focus:ring-indigo-500 focus:border-indigo-500 @error('down_payment') border-red-500 @enderror transition-colors duration-300">
